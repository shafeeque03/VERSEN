...
const insertUser = async (req, res) => {
  try {
    ...
    if (already) {
      ...
      if (findUser) {
        const addMoneyUser = await User.findOneAndUpdate(
          { referCode: referCode },
          { $inc: { wallet: 100 } }
        );
        const addMoneyNewUser = await User.findByIdAndUpdate(
          { _id: newUserID },
          { $inc: { wallet: 100 } }
        );
...
const otpVerifying = async (req, res) => {
  try {
    ...
    if (OTP == otp) {
      ...
      res.render("otp_page", { message: "Enter a valid OTP", userData });
    }
  } catch (error) {}
};
  ...
  try {
    ...
    if (orderData.payment == "razor") {
      ...
      const cancel = await Order.findOneAndUpdate(
        { user: req.session.user_id, "products._id": id },
        {
          $set: {
...
const returnOrder = async (req, res) => {
  try {
    ...
    const returned = await Order.findOneAndUpdate(
      { user: req.session.user_id, "products._id": pid },
      {
        $set: {
